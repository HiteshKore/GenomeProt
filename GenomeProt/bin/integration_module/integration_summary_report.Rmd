...
txuniqtotal <-nrow(df%>%dplyr::filter(transcript_identified == TRUE)%>%select("transcript_id")%>%unique())
  
  
  nrow(df%>%dplyr::filter(peptide_ids_transcript == TRUE)%>%select("peptide")%>%unique())
  

genetotal <- length(unique(df$gene_id))

geneuniqtotal <- df %>%dplyr::filter(gene_identified==TRUE)%>%select("gene_id")%>%unique()%>%NROW

orftotal <- unique(df$accession)%>%NROW

orfuniqtotal <- df %>% dplyr::filter(orf_identified == TRUE)%>%select(accession)%>%unique()%>%NROW
...
a <- df%>%dplyr::filter(peptide_ids_orf==TRUE)%>%select(peptide)%>%unique()%>%NROW
#Transcript-specific peptides   
b <- df %>% dplyr::filter(peptide_ids_transcript==TRUE)%>%select(peptide)%>%unique()%>%NROW
#Gene-specific peptides            
c <- df %>% dplyr::filter(peptide_ids_gene==TRUE)%>%select(peptide)%>%unique()%>%NROW
#Peptides spanning splice junctions 
d <- df%>%dplyr::filter(number_of_mapped_exons > 1)%>%select(peptide)%>%unique()%>%NROW
...
f <- df %>% filter(!startsWith(transcript_id, "Bambu"))%>%select(transcript_id)%>%unique()%>%NROW
#Novel transcripts with peptide evidence 
g <- df %>% filter(startsWith(transcript_id, "Bambu"))%>%select(transcript_id)%>%unique()%>%NROW
  
#Known transcripts encoding ORFs with UMPs 
h <- df %>% dplyr::filter(orf_identified == TRUE)%>%filter(!startsWith(transcript_id, "Bambu"))%>%select(transcript_id)%>%unique()%>%NROW

#Novel transcripts encoding ORFs with UMPs 
i <- df %>% dplyr::filter(orf_identified == TRUE)%>%filter(startsWith(transcript_id, "Bambu"))%>%select(transcript_id)%>%unique()%>%NROW

#Peptide-confirmed known transcripts 
j <- df %>% dplyr::filter(transcript_identified == TRUE & !startsWith(transcript_id, "Bambu"))%>%select(transcript_id)%>%unique()%>%NROW
  
#Peptide-confirmed novel transcripts
k <-df %>% dplyr::filter(transcript_identified == TRUE & startsWith(transcript_id, "Bambu"))%>%select(transcript_id)%>%unique()%>%NROW
  
#Peptide-confirmed novel transcripts encoding annotated ORFs with UMPs
l <- df %>% dplyr::filter(transcript_identified == TRUE & startsWith(transcript_id, "Bambu"))%>%dplyr::filter(!startsWith(accession, "ORF_"))%>%select(accession)%>%unique()%>%NROW

#Peptide-confirmed novel transcripts encoding unannotated ORFs with UMPs 	
m <- df %>% dplyr::filter(transcript_identified == TRUE & startsWith(transcript_id, "Bambu"))%>%dplyr::filter(startsWith(accession, "ORF_"))%>%select(accession)%>%unique()%>%NROW
...
n <-df %>% dplyr::filter(!startsWith(accession, "ORF_"))%>%select(accession)%>%unique()%>%NROW

#Unannotated ORFs with peptide evidence 	 
o <-df %>% dplyr::filter(startsWith(accession, "ORF_"))%>%select(accession)%>%unique()%>%NROW

#Annotated ORFs with UMPs
p <-df %>% dplyr::filter(orf_identified == TRUE & !startsWith(accession, "ORF_"))%>%select(accession)%>%unique()%>%NROW
  
#Unannotated ORFs with UMPs 
q <-df %>% dplyr::filter(orf_identified == TRUE & startsWith(accession, "ORF_"))%>%select(accession)%>%unique()%>%NROW
...
summary_df <- df %>%dplyr::filter(peptide_ids_orf==TRUE & startsWith(accession, "ORF"))%>%select(accession,localisation)%>%unique()%>%
...
summary_df <- df %>% dplyr::filter(orf_identified == TRUE) %>% 
            select(accession,protein_length,orf_type) %>%unique()%>%
