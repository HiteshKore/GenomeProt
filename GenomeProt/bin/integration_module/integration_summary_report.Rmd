...
df_var_peptides=df%>%filter(grepl("_var",accession))
...
uniqpeptides <- nrow(df %>% group_by(peptide) %>%
            dplyr::filter(length(unique(accession))==1) %>%
            slice_head(n=1))
...
#Number of variant ORFs with UMPs
  var_p<-df_var_peptides%>%filter(peptide_ids_orf==TRUE)%>%dplyr::select(accession)%>%unique()%>%NROW
#Number of canonical variant ORFs with UMPs
var_p_canonical<-df_var_peptides%>%filter(peptide_ids_orf==TRUE)%>%dplyr::select(accession)%>%dplyr::filter(!startsWith(accession, "ORF_"))%>%unique()%>%NROW

#Number of canonical variant ORFs with UMPs
var_p_novel<-df_var_peptides%>%filter(peptide_ids_orf==TRUE)%>%dplyr::select(accession)%>%dplyr::filter(startsWith(accession, "ORF_"))%>%unique()%>%NROW

#Number of variant protein specific UMPs
var_pep<-df_var_peptides%>%filter(peptide_ids_orf==TRUE)%>%dplyr::select(peptide)%>%unique()%>%NROW

#Number of canonical variant protein specific UMPs
var_pep_novel<-df_var_peptides%>%filter(peptide_ids_orf==TRUE)%>%dplyr::select(accession,peptide)%>%dplyr::filter(startsWith(accession, "ORF_"))%>%dplyr::select(peptide)%>%unique()%>%NROW

#Number of canonical variant protein specific UMPs
var_pep_known<-df_var_peptides%>%filter(peptide_ids_orf==TRUE)%>%dplyr::select(accession,peptide)%>%dplyr::filter(!startsWith(accession, "ORF_"))%>%dplyr::select(peptide)%>%unique()%>%NROW

item_var <- c(
  "Variant proteins with UMPs ",
