...
df <- fread("../integ_output/peptide_info.csv")
...
isouniqtotal <- nrow(df %>% group_by(transcript_id) %>% 
            dplyr::filter(pep_map_status == "high") %>% slice_head(n=1))
genetotal <- length(unique(df$gene))
...
b <- nrow(df %>% group_by(peptide) %>% 
            dplyr::filter(n()==1 & pep_map_status == "high") %>% 
            slice_head(n=1))
c <- nrow(df %>% group_by(peptide) %>% 
            dplyr::filter(length(unique(PID))==1 & length(unique(gene))==1 & 
            pep_map_status == "high") %>% 
            slice_head(n=1))
...
List of novel isoforms distinguished with uniquely mapping peptides: `r paste0(unique(c(dist_nov_iso$transcript_id)))`
...
summary_df <- df %>% dplyr::filter(pep_map_status == "high") %>% 
...
#### Plot of correlation between peptide intensity and isoform expression
